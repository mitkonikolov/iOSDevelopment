...
class SetCardView: UIView {
  ...
  // view properties
  private var shape: Shape = Shape.square {
    didSet {
      setNeedsDisplay()
      setNeedsLayout()
    }
  ...
  @IBInspectable
  private var numberSymbols: Int = 3 {
    didSet {
      setNeedsDisplay()
      setNeedsLayout()
    }
  ...
  private var shading: Shading = Shading.striped {
    didSet {
      setNeedsDisplay()
      setNeedsLayout()
    }
  ...
  public var state = CardState.normal {
    didSet {
      setNeedsDisplay()
      setNeedsLayout()
    }
  ...
  private var firstSubviewYCoordinate: CGFloat {
    let viewsTotalHeight = CGFloat(numberSymbols) * subviewSide
      * subviewSideSpacingRatio
